...
import { isInsidePreviewSourceBlock, skipPreviewSourceBackward, skipPreviewSourceForward } from '../../block/previewSource';
...
export class ParagraphFrontMenu extends BaseFloat {
    ...
    selectItem(event: Event, { label }: { label: string }) {
        ...
        // 图片是内联元素，不走「区块转换」；交由宿主处理「选文件→转存→插入」。
        // 宿主通过 muya-factory 监听 muya-insert-image 事件弹出图片插入对话框。
        const cursorBlock = label === 'image'
            ? this._insertImage(block)
            : /duplicate|new|delete/.test(label)
    ...
    private _insertImage(block: Parent): null {
        // 先把光标放进触发段落，图片会插入到该光标位置
        block.firstContentInDescendant()?.setCursor(0, 0, true);
        this.muya.eventCenter.emit('muya-insert-image', {});
        return null;
    }
    ...
    private _applyMetaAction(label: string, block: Parent, oldState: TState) {
        ...
        switch (label) {
            case 'duplicate': {
                ...
                const caret = dupBlock.lastContentInDescendant();
                // A caret inside an html / math / diagram block flips it to
                // raw-source view; leave the duplicated block rendered.
                return caret && isInsidePreviewSourceBlock(caret) ? null : caret;
            }
            ...
            case 'delete': {
                // Skip html / math / diagram blocks when choosing the caret
                // target: a caret dropped into one flips it to raw-source
                // view right after the deletion.
                let cursorBlock: Content | null = null;
                if (block.prev) {
                    cursorBlock = skipPreviewSourceBackward(block.prev.lastContentInDescendant());
                }
                if (cursorBlock == null && block.next) {
                    cursorBlock = skipPreviewSourceForward(block.next.firstContentInDescendant());
                }
                if (cursorBlock == null) {
                    const state = deepClone(emptyStates.paragraph);
                    const newBlock = ScrollPage.loadBlock('paragraph').create(
