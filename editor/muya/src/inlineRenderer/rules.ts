...
export const commonMarkRules = {
    ...
    /^(<!--[\s\S]*?-->|(<([a-z][a-z\d-]*)[^\n<>]*>)(?:([\s\S]*?)(<\/\3 *>))?)/i, // raw html
    // Numeric character references (&#39; / &#x27;) first, then named ones.
    html_escape: new RegExp(`^(&#x[0-9a-f]{1,6};|&#\\d{1,7};|${escapeCharacters.join('|')})`, 'i'),
    soft_line_break: /^(\n)(?!\n)/,
    hard_line_break: /^( {2,})(\n)(?!\n)/,
...
export const inlineExtensionRules = {
    // 与 MarkText 的 marked inlineRule 一致：允许 `$$…$$` 双美元符行内公式
    // （displayMode），否则段落中间的 $$x=1$$ 会原样显示为文本。
    // eslint-disable-next-line regexp/no-super-linear-backtracking
    inline_math: /^(\${1,2})((?:[^$\\]|\\.)+)(\\*)\1(?!\1)/,
