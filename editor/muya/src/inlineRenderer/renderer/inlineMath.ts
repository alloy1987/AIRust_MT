...
export default function inlineMath(this: Renderer, {
    ...
    // `$$…$$` renders in display mode, mirroring the marked math extension
    // (`displayMode: match[1].length === 2`).
    const displayMode = marker.length === 2;
    // Cache per marker: `$x$` and `$$x$$` share content `x` but need
    // different KaTeX displayMode output.
    const key = `${math}_${type}_${marker}`;
    let mathVnode = null;
