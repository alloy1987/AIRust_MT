...
export function getImageSrc(src: string) {
    ...
    if (imageExtension) {
        ...
        // Host hook (Tauri: `convertFileSrc`) that maps a local filesystem path
        // to a webview-loadable URL (`asset://…`). When absent we fall back to
        // the plain `file://` form (headless / browser).
        const convert
            = typeof window !== 'undefined'
                ? (window as unknown as { __convertFileSrc?: (p: string) => string }).__convertFileSrc
                : undefined;
        if (isUrl) {
            return {
                isUnknownType: false,
            ...
            const localPath = resolveRelativePath(baseUrl, src);
            return {
                isUnknownType: false,
                src: convert ? convert(localPath) : `file://${localPath}`,
            };
        }
        else {
            return {
                isUnknownType: false,
                src: convert ? convert(src) : `file://${src}`,
            };
        }
    }
