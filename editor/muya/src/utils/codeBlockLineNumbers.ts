...
export function repositionLineNumberSpans(
    ...
    // Range.getBoundingClientRect() reports viewport coordinates, i.e.
    // already multiplied by any CSS `zoom` applied to the editor host. The
    // `top` values we assign are laid out *inside* the zoomed subtree, so
    // they would be scaled a second time — making the gutter over- or
    // under-shoot the code block at zoom !== 1. Normalize by the zoom scale
    // of the nearest zoomed ancestor.
    const zoom = nearestZoomScale(codeEl);
    ...
    while (node !== null && lineIdx < lineStarts.length) {
        ...
        while (lineIdx < lineStarts.length && lineStarts[lineIdx] < nodeEnd) {
            ...
            range.collapse(true);
            const measured = range.getBoundingClientRect().top / zoom;
            if (baseTop === null)
                baseTop = measured;
    ...
    // their predecessor. `lineHeight` is an unscaled CSS value, matching the
    // unscaled `top` offsets above.
    if (lineIdx < spans.length) {
...
// Find the CSS `zoom` scale of the nearest zoomed ancestor (the app scales
// the whole editor host, so in practice this is a single factor). A stack of
// nested zoomed containers is not supported by this helper — the editor only
// ever zooms one level.
function nearestZoomScale(el: HTMLElement): number {
    let node: HTMLElement | null = el;
    while (node) {
        const zoom = Number.parseFloat(getComputedStyle(node).zoom);
