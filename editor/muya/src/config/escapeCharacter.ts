...
// CommonMark §2.5 numeric character references: `&#39;` / `&#x27;` (hex is
// case-insensitive). Out-of-range / surrogate code points become U+FFFD.
const numericEntityRule = /^&#(?:x([0-9a-f]{1,6})|(\d{1,7}));$/i;

export function decodeHtmlEntity(entity: string): string | undefined {
    const named = escapeCharactersMap[entity];
    if (named !== undefined)
        return named;

    const match = numericEntityRule.exec(entity);
    if (!match)
        return undefined;
