...
import { skipPreviewSourceForward } from '../block/previewSource';
...
export class Editor {
    ...
    focus() {
        ...
        // A caret inside an html / math / diagram block flips it to its
        // raw-source view, so a document opening on such a block would show
        // a code block instead of the rendered preview. Skip past them; when
        // nothing else is editable, leave the caret unplaced so the preview
        // stays visible (clicking the block still enters source editing).
        let firstLeafBlock = skipPreviewSourceForward(scrollPage?.firstContentInDescendant() ?? null);
